...
An authorization server that cannot be read raises `ERClientServiceUnreachable` — a document that does not parse or does not name the issuer we asked about, a device-authorization response missing anything the flow needs, a verification URI that is not absolute `https`, and an approval whose token response lacks the token, its type or its lifetime all count. Nothing was refused on those paths, so `last_auth_error` is left as it was rather than describing them.
