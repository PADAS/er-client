...
An authorization server that cannot be read raises `ERClientServiceUnreachable` — a document that does not parse or does not name the issuer we asked about, a device-authorization response missing anything the flow needs, a verification URI that is not absolute `https`, an approval whose token response lacks the token, its type or its lifetime, and a token whose readable `iss` is not the issuer the flow ran against all count. Nothing was refused on those paths, so `last_auth_error` is left as it was rather than describing them. A network failure while talking to the authorization server is wrapped the same way for the metadata fetch only; on the device-authorization request and the token poll it propagates as the HTTP library's own exception (`requests.RequestException` or `httpx.RequestError`), exactly as it does for the password grant's token request.
...
Discovery and the device-code requests use their own timeouts — five seconds for the two metadata reads, ten for the two device-code POSTs — on both clients. These are per-request timeouts applied to each network phase, not end-to-end deadlines: a slow connect followed by a slow read can take longer than the figure, and the async client's transport retries connection failures up to `max_http_retries` times before giving up. `connect_timeout` and `data_timeout` shape the async client's API calls and do not apply here.
...
The warnings and the two legacy-credential refusals name the way out: pass an Auth0-issued token, or construct the client with no credentials and call `login()` to sign in interactively. The refusal for an Auth0 token from an unlisted issuer instead lists the issuers the site does accept, since the way out there is a token minted by one of them.

The warning rows change nothing — what worked before still works, and a failing login still raises exactly what it raised before. The raise rows are the cases the site's API would reject on every call: sync `login()` returns `False` without posting and `auth_headers()` raises, async `login()` raises directly, and in `token=` mode every `auth_headers()` raises. For a client that brought credentials, discovery itself never blocks auth: a 404, a 5xx, a malformed document or an unreachable endpoint all just mean "no metadata", logged at DEBUG, and no metadata means no warning and no refusal. A client that brought none is the other way round — it has nowhere to sign in without that document, so no metadata is itself the refusal, and `discovery=False` is one too unless `device_code_issuer=` says where to go. Pass `discovery=False` to the constructor to switch off the automatic fetches, the warnings and the refusals for a client with credentials; `client.discover()` still works, returning `ProtectedResourceMetadata | None`, and `client.protected_resource_metadata` holds whatever the last fetch found.
...
| anything else (unknown code, or no OAuth `error` with a status not listed above) | `ERClientException` |
