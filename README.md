...
For the reason behind a `False`, read `client.last_auth_error` — an `AuthError` carrying `status_code`, `error`, `error_description`, `response_body`, `url`, `grant_type` and `retry_after`. It is `None` until a token request is refused, and is cleared by the next successful one. It is also set when the client refuses credentials itself, before sending anything (see "What the site tells the client" above); `status_code` and `response_body` are then `None`, since no server was consulted, and the exception's message is the whole explanation rather than `Login failed.`. It is available on both clients.
