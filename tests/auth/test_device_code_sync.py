...
class TestTheHappyPath:
    ...
    def test_a_refresh_token_the_tenant_sent_anyway_is_dropped(
        self, service_root, fake_device_server, no_sleep, captured_prompt,
        device_token_response, make_requests_response, default_token_url,
    ):
        ...
        client = ERClient(service_root=service_root,
                          device_code_prompt=captured_prompt.append)
        server = fake_device_server(token_responses=[make_requests_response(
...
class TestThereIsNoTenantToSignInAgainst:
    ...
    @pytest.mark.parametrize(
        "issuer",
        ["http://someone-elses-tenant.us.auth0.com",
         f"{OTHER_ISSUER}?tenant=x", f"{OTHER_ISSUER}#fragment"],
        ids=["plain_http", "query", "fragment"],
    )
    def test_an_issuer_override_that_is_not_an_issuer_url(
        self, service_root, patched_get, patched_post, issuer,
    ):
        """Its metadata names where credentials go, so it is never fetched in
        the clear, nor from a URL the well-known path cannot be appended to —
        not even with the whole registration supplied."""
        client = ERClient(service_root=service_root,
                          device_code_issuer=issuer,
                          device_code_client_id="new-client",
                          device_code_audience="https://new.example.org/api")

        self.assert_refused(client, INVALID_OVERRIDE_MESSAGE)
        assert not patched_get.called
        assert not patched_post.called
